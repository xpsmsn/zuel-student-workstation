...
| --- | --- | --- |
| `ZUEL-StudentWorkstation-v1.9.8-Portable.exe` | 中南大学生工作台-v1.9.8-绿色版.exe | 想直接用：复制到任意文件夹双击即可，**不用安装** |
| `ZUEL-StudentWorkstation-v1.9.8-Setup.exe` | 中南大学生工作台-v1.9.8-安装程序.exe | 日常使用：装到开始菜单/桌面。默认按当前用户安装，**不需要管理员权限** |
| `ZUEL-StudentWorkstation-v1.9.8-Deploy.msi` | 中南大学生工作台-v1.9.8-单位部署.msi | 学校统一部署用，通常需要管理员权限 |
